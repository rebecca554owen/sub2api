...
The bundled Compose service can run a separate ClickHouse instance for sub2api:

```bash
COMPOSE_PROFILES=clickhouse
CLICKHOUSE_DB=sub2api_logs
CLICKHOUSE_USER=sub2api_logger
CLICKHOUSE_PASSWORD=replace-with-a-long-random-password
LOG_SQL_DSN=clickhouse://sub2api_logger:replace-with-a-long-random-password@clickhouse:9000/sub2api_logs?compress=lz4
```

It stores data under the sub2api deployment (`clickhouse/data` for the local
Compose file) and does not share a database, credentials, container, or network
with New API.
